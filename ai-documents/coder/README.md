...
| 011 | Fix the 009 C2 destination-eviction path (it hangs the board) | **open — needs decisions D1–D4 before the coder starts** (2026-09-24). Six leads, one certain (`evictW` is compile-time, so **random mode runs the new path** — contradicts 009's U2). Gate is 4 consecutive PLRU board completions, not a green sim |
