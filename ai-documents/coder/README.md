...
| 009 | A migration evicts D's least-recent line whatever its state (probe + write-back), PLRU mode only, no new register or counter | **open — ready to start** (2026-09-18). Two RTL commits: client-free W; then client-held W with the probe and the D-nest (P1 shape). Board: 64 KB PLRU image |
