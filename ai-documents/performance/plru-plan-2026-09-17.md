...
**Written 2026-09-17** by the thinker. **Status: decided 2026-09-17 — D1 runtime register, D2 staged
(abort + count by reason now; full destination eviction is task 009), D3 accesses + installs.** The work
order is [coder/008 TASK.md](../coder/008-plru-replacement/TASK.md); **where they differ, the TASK wins**
(register moved to `0x490` because `0x470`–`0x480` are reserved by task 005; V1 no longer compares counters
across builds, see 005 F2).
...
- **Order decided 2026-09-17: PLRU first.** The heat-counter gaps against the paper (fix plan §1 rows 7,
  9–12, tracker M17/M18) are **not** fixed before this. They are the next optimization lever if PLRU does
  not win. Both A/B halves run on today's counter logic, so the comparison stays fair.
...
flowchart TB
  ...
  subgraph CTRL["Control.scala"]
    REPL["L2_Replacement at 0x490<br/>0 = random, 1 = PLRU"]
  end
...
- `L2_Replacement` at **`0x490`**, R/W, 1 bit, **reset 0 = random**, 1 = PLRU. (`0x430` is reserved for
  `SBC_Declined`, `0x440`–`0x480` for the 005 sampler.)
...
|---|---|---|---|
| **D1** | How to choose the policy | **Runtime register `0x490`** — all four halves on one image and one RTL; one 64 KB build; costs a 2:1 mux | Compile-time only: simpler, no register or SW change, but two bitstreams per size. The 09-16 "no register" rule was about four muxes on the SBU path. This is one mux in the directory. |
...
| # | Check | Pass |
|---|---|---|
| V1 | **Nothing changes while random is selected.** Flag on, register left at reset. Re-run the `007-b71-c3` gate with the **same binaries** | ~~every counter identical~~ **corrected:** the stress test is not reproducible across builds (005 F2), so only the deterministic switch test must match `007-b71-c3`; plus byte-identical Verilog with the flag off (TASK V0). 8/8 + 7/7, T1–T4, 0 asserts, shadows quiet |
| V2 | **PLRU gate.** Same tests built with `-DL2_POLICY=1` (writes `0x490` first), plus a `=0` build of the same source as the reference | 8/8 + 7/7, T1–T4, 0 asserts, shadows quiet; the counter identities hold |
