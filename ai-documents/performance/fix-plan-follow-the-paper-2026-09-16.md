...
| 7 | Native miss in S is +1 for S even when the second search hits; secondary hit is −1 for **D** (Fig 2: set 2 goes 1 → 0); a definitive miss also updates D (§3.3 "in both cases", so +1 — text only, no figure); installing a guest does not move D (Fig 2: set 2 stays 1) | S +1 ✅, D untouched ❌ | ❌ **no longer inert (2026-09-17):** C3 teardown is built, so D's counter now decides whether D is picked again or becomes a source after the pair ends |
