 ...
 * The outer-port and lookup groups are NOT gated by enableSetBalancing - an SBC-on vs SBC-off A/B
 * needs them counting identically in both builds.
 */
...
class PerfCounters(params: InclusiveCacheParameters) extends Module
{
  ...
  when (io.aFire && io.aOpcode === TLMessages.AcquirePerm)  { memAcqPerm  := memAcqPerm  + 1.U }
  ...
  // ---- outcomes (005 commit 1) ----

  // After the increments, so a same-cycle clear wins (drops that cycle's events).
  when (io.clearStats) {
    memReads    := 0.U
    memWrites   := 0.U
    memAcqPerm  := 0.U
  ...
  if (params.micro.enableSetBalancing) {
    ...
    // A level, not an event: never cleared. A commit and an erase in one cycle cancel; never below 0.
