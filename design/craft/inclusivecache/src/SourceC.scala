...
class SourceC(params: InclusiveCacheParameters) extends Module
{
  val io = IO(new Bundle {
    ...
    // 012 C2 (H3): high from accepting a dirty Release until its last beat's read address has fired,
    // i.e. while this module is still READING evict_req's row. The migration copy must not write
    // that row yet. `busy` is a register, and while it is set `req` is the latched request, so
    // evict_req names the row being read and not whatever the Scheduler is presenting this cycle.
